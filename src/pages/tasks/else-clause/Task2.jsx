...
export default function Task2({ onSuccess }) {
  ...
  const [input, setInput] = useState("3 9");
...
`);
      ...
      const printsOK = resultOutput.includes("OK");

      if (hasWhile && hasElse && hasBreak && printsOK) {
        setIsCorrect(true);
      } else {
        setIsCorrect(false);
      }
  ...
  return (
    <div style={{ backgroundColor: "#000", color: "#FFD700", minHeight: "100vh", padding: "2rem" }}>
      ...
      <p style={{ color: "#ccc", fontSize: "1.2rem", marginBottom: "1rem" }}>
        🧠 a және b сандары берілген. Осы аралықтағы барлық натурал сандардың квадраттарын шығаратын бағдарлама жаз. Егер квадрат 8-ге бөлінсе, циклді тоқтат. Әйтпесе <code>OK!</code> деп жаз.
        <br /> <strong>Кеңес:</strong> <code>while-else</code> құрылымын қолдан.
      </p>
      ...
      {isCorrect && (
        <div style={{ marginTop: "2rem" }}>
          ...
          <button
            onClick={() => navigate("/grade8/else-clause/3")}
            style={{
