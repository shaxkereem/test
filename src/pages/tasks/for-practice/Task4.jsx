...
`);
      ...
      const hasPower = normalizedCode.includes("**3") || normalizedCode.includes("pow(i,3)");
      const isExpected = resultOutput === "100" || resultOutput.endsWith("100");

      if (hasFor && hasPower && isExpected) {
        setIsCorrect(true);
      } else {
        setIsCorrect(false);
      }
  ...
  return (
    <div style={{ backgroundColor: "#000", color: "#FFD700", minHeight: "100vh", padding: "2rem" }}>
      ...
      <p style={{ color: "#ccc", fontSize: "1.2rem", marginBottom: "1rem" }}>
        🔢 Пайдаланушы енгізген <code>n</code> санына дейінгі барлық сандардың кубтарының қосындысын есептейтін бағдарлама жаз.<br />
        Формула: 1<sup>3</sup> + 2<sup>3</sup> + 3<sup>3</sup> + ... + n<sup>3</sup>
      </p>
      ...
      {isCorrect && (
        <div style={{ marginTop: "2rem" }}>
          {!pointAdded && (
            <button onClick={handleAddPoint} style={{ ...buttonStyle, backgroundColor: "#0f0", color: "#000", marginRight: "1rem" }}>
              ⭐ Ұпай қосу
            </button>
          )}
          <button
