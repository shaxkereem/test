...
`);
      ...
      const hasMul = normalizedCode.includes("*=") || normalizedCode.includes("=" && "*");

      const expected = parseInt(input);
      let expectedFactorial = 1;
      for (let i = 1; i <= expected; i++) expectedFactorial *= i;

      if (hasFor && hasMul && resultOutput === expectedFactorial.toString()) {
        setIsCorrect(true);
      } else {
        setIsCorrect(false);
      }
  ...
  return (
    <div style={{ backgroundColor: "#000", color: "#FFD700", minHeight: "100vh", padding: "2rem" }}>
      ...
      <p style={{ color: "#ccc", fontSize: "1.2rem", marginBottom: "1rem" }}>
        Пайдаланушы бір оң сан енгізеді. Сол санның факториалын (n!) есептейтін бағдарлама жаз.
      </p>
