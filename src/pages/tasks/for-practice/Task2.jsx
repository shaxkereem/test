...
`);
      ...
      const hasPrintStar = code.includes("*") && code.includes("print");
      const correctLine = resultOutput.includes("* * * * * *");

      if (hasFor && hasPrintStar && correctLine) {
        setIsCorrect(true);
      } else {
        setIsCorrect(false);
      }
  ...
  return (
    <div style={{ backgroundColor: "#000", color: "#FFD700", minHeight: "100vh", padding: "2rem" }}>
      ...
      <p style={{ color: "#ccc", fontSize: "1.2rem", marginBottom: "1rem" }}>
        🔺 Айдос ⭐ жұлдызшалардан үшбұрыш салғысы келеді. <br />
        <strong>for</strong> циклін қолданып, төмендегідей үлгіні шығаратын бағдарлама жаз:
      </p>

      <div style={{ backgroundColor: "#111", color: "#FFD700", padding: "1rem", borderRadius: "10px", fontFamily: "monospace", marginBottom: "1rem" }}>
        <pre>{`Enter the number of rows: 6
*
* *
* * *
* * * *
...
* * * * * *`}</pre>
      ...
      {isCorrect && (
        <div style={{ marginTop: "2rem" }}>
          ...
          <button
            onClick={() => navigate("/grade8/for-practice/3")}
            style={{
