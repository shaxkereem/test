...
export default function Task2({ onSuccess }) {
  ...
  const [input, setInput] = useState("1 20");
...
`);
      ...
      const hasDivCheck = normalizedCode.includes("%3") || normalizedCode.includes("mod3");
      const hasExpected = resultOutput.includes("3") && resultOutput.includes("6") && resultOutput.includes("9");

      if (hasWhile && hasDivCheck && hasExpected) {
        setIsCorrect(true);
      } else {
        setIsCorrect(false);
      }
  ...
  return (
    <div style={{ backgroundColor: "#000", color: "#FFD700", minHeight: "100vh", padding: "2rem" }}>
      ...
      <p style={{ color: "#ccc", fontSize: "1.2rem", marginBottom: "1rem" }}>
        👦 Айдос жаңа Python ойынын жазып жатыр. Екі сан арасындағы 3-ке бөлінетін сандарды табуы қажет.
        Бірақ ол бұл кодты аяқтай алмай жатыр. Сен көмектесе аласың ба?
      </p>
