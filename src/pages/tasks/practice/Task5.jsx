...
export default function Task5({ onSuccess }) {
  ...
  const [input, setInput] = useState("10 35 5");
...
def input(prompt=None):
    return input_values.pop(0)
      ...
      const hasRange = normalizedCode.includes("range");
      const correctOutput = resultOutput.includes("10") && resultOutput.includes("35");

      if (hasFor && hasRange && correctOutput) {
        setIsCorrect(true);
      } else {
        setIsCorrect(false);
      }
  ...
  return (
    <div style={{ backgroundColor: "#000", color: "#FFD700", minHeight: "100vh", padding: "2rem" }}>
      ...
      <p style={{ color: "#ccc", fontSize: "1.2rem", marginBottom: "1rem" }}>
        🧮 Пайдаланушыдан үш бүтін сан енгізіледі: минимум мәні, максимум мәні және қадам.
        Осы мәндерге сәйкес барлық натурал сандарды шығаратын программа жаз.
        <br />
        <strong>Мысалы:</strong> Егер кіріс <code>10 35 5</code> болса, нәтиже <code>10 15 20 25 30 35</code>
      </p>
      ...
      {isCorrect && (
        <div style={{ marginTop: "2rem" }}>
          ...
          <button
            onClick={() => navigate("/grade8/general-practice/1")}
            style={{
