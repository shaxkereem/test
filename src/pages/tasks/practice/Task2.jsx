...
`);
      ...
      const usedMult = normalizedCode.includes("*2") || normalizedCode.includes("=2*");
      const correctOutput = resultOutput.includes("1") && resultOutput.includes("2") && resultOutput.includes("4") && resultOutput.includes("8") && resultOutput.includes("16") && resultOutput.includes("32") && !resultOutput.includes("64");

      if (hasWhile && usedMult && correctOutput) {
        setIsCorrect(true);
      } else {
        setIsCorrect(false);
      }
  ...
  return (
    <div style={{ backgroundColor: "#000", color: "#FFD700", minHeight: "100vh", padding: "2rem" }}>
      <h1 style={{ fontSize: "2rem", marginBottom: "1rem" }}>⚙️ Тапсырма 2</h1>

      <p style={{ color: "#ccc", fontSize: "1.2rem", marginBottom: "1rem" }}>
        🧠 Компьютерлер барлық ақпаратты екілік жүйеде (биттермен) сақтайды. Берілген саннан аспайтын 2-нің барлық дәрежелерін <strong>көбейту амалын</strong> қолданып шығаратын бағдарлама жаз.
      </p>

      <div style={{ backgroundColor: "#111", color: "#FFD700", padding: "1rem", borderRadius: "10px", fontFamily: "monospace", marginBottom: "1rem" }}>
        <pre>{`Кіріс: 60\nШығыс: 1 2 4 8 16 32`}</pre>
      </div>
      ...
      {isCorrect && (
        <div style={{ marginTop: "2rem" }}>
          ...
          <button
            onClick={() => navigate("/grade8/general-practice/3")}
            style={{
