...
`);
      ...
      const normalized = resultOutput.toLowerCase();
      const correct = input === "11"
        ? normalized.includes("prime")
        : input === "407"
        ? normalized.includes("not") && normalized.includes("prime")
        : normalized.includes("prime") || normalized.includes("not");

      const hasForOrWhile = code.includes("for") || code.includes("while");
      const hasIf = code.includes("if");

      if (correct && hasIf && hasForOrWhile) {
        setIsCorrect(true);
      } else {
        setIsCorrect(false);
      }
  ...
  return (
    <div style={{ backgroundColor: "#000", color: "#FFD700", minHeight: "100vh", padding: "2rem" }}>
      ...
      <p style={{ color: "#ccc", fontSize: "1.2rem", marginBottom: "1rem" }}>
        Пайдаланушы бір бүтін сан енгізеді. Сол сан жай сан (prime) екенін анықтайтын Python бағдарламасын жаз.
      </p>
...
Output: 407 is not a prime number`}
      ...
      {isCorrect && (
        <div style={{ marginTop: "2rem" }}>
          ...
          <button
            onClick={() => navigate("/grade8/for-practice/4")}
            style={{
