...
export default function Task3({ onSuccess }) {
  ...
  const runCode = async () => {
    ...
    try {
      ...
      const hasConditions = normalizedCode.includes("%7==0") && normalizedCode.includes("%5==0");
      const expectedSample = resultOutput.includes("1505") || resultOutput.includes("1680") || resultOutput.includes("2100");

      if (hasFor && hasConditions && expectedSample) {
        setIsCorrect(true);
      } else {
        setIsCorrect(false);
      }
  ...
  return (
    <div style={{ backgroundColor: "#000", color: "#FFD700", minHeight: "100vh", padding: "2rem" }}>
      ...
      <p style={{ color: "#ccc", fontSize: "1.2rem", marginBottom: "1rem" }}>
        ✨ 1500 бен 2700 аралығындағы 7-ге бөлінетін және 5-ке еселік сандарды табатын Python бағдарламасын жаз.
      </p>
