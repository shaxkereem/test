...
`);
      ...
      const hasModulus = normalizedCode.includes("%");
      const hasExpected = resultOutput.includes("6"); // Example for input 48 18 GCD is 6

      if (hasWhile && hasModulus && hasExpected) {
        setIsCorrect(true);
      } else {
        setIsCorrect(false);
      }
  ...
  return (
    <div style={{ backgroundColor: "#000", color: "#FFD700", minHeight: "100vh", padding: "2rem" }}>
      <h1 style={{ fontSize: "2rem", marginBottom: "1rem" }}>🧮 Practice 3 - Euclid's Algorithm</h1>

      <p style={{ color: "#ccc", fontSize: "1.2rem", marginBottom: "1rem" }}>
        ✨ Екі бүтін сан A және B берілген. Euclid алгоритмін қолданып, олардың ең үлкен ортақ бөлгішін (GCD) тап.
        <br />
        🔎 Мысалға:
      </p>
