...
export default function Task1({ onSuccess }) {
  ...
  const runCode = async () => {
    ...
    try {
      ...
      // input функциясын қайта анықтау
...
`);
      ...
      // Тексеру логикасы
      const expectedOutput = Array.from({ length: parseInt(input) }, (_, i) => `${i + 1}`).join("\n");

      const normalizedCode = code.replace(/\s/g, "").toLowerCase();
      const hasFor = normalizedCode.includes("for");
      const hasPrint = normalizedCode.includes("print");

      if (hasFor && hasPrint && resultOutput === expectedOutput) {
        setIsCorrect(true);
      } else {
        setIsCorrect(false);
      }
  ...
  return (
    <div style={{ backgroundColor: "#000", color: "#FFD700", minHeight: "100vh", padding: "2rem" }}>
      ...
      <p style={{ color: "#ccc", fontSize: "1.2rem", marginBottom: "1rem" }}>
        Берілген <strong>N</strong> санына дейінгі барлық натурал сандарды (1-ден N-ге дейін) шығару қажет. Нәтижені әр сан жаңа жолға шығарып отыр.
      </p>
...
5`}
      ...
      {isCorrect && (
        <div style={{ marginTop: "2rem" }}>
          ...
          <button
            onClick={() => navigate("/grade8/for-practice/2")}
            style={{
